...
const SectionEntities = ({ notify, isDark }) => {
  ...
  // ── Row item ───────────────────────────────────────────────────
  const RowItem = ({ label, sub, onEdit, onDelete }) => (
    <div className={`flex items-center justify-between px-4 py-3 ${t.innerBg} rounded-xl border ${t.innerBorder} hover:border-[#5478FF]/40 transition-all group`}>
      ...
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={onEdit}   className="p-1.5 rounded-lg text-sky-500 hover:bg-sky-500/10"><Pencil size={13}/></button>
        <button onClick={onDelete} className="p-1.5 rounded-lg text-red-500 hover:bg-red-500/10"><Trash2 size={13}/></button>
      </div>
    </div>
  );
          ...
          : <div className="space-y-2">
              {subjects.map(s => (
                <RowItem key={s.subjectId} label={`${s.subjectCode} — ${s.subjectName}`} sub={`${s.credits} credits`}
                  ...
                  onDelete={() => setSubConfirm(s.subjectId)}/>
              ))}
            </div>
        }
      </AccordionSection>
